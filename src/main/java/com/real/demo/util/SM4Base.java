...
public class SM4Base {
    ...
    private long GET_LONG_BE(byte[] b, int i) {
        return (long) (b[i] & 0xff) << 24 | (long) ((b[i + 1] & 0xff) << 16) | (long) ((b[i + 2] & 0xff) << 8)
    ...
    private long sm4Lt(long ka) {
        ...
        b[3] = sm4Box(a[3]);
        bb = GET_LONG_BE(b, 0);
        c = bb ^ ROT(bb, 2) ^ ROT(bb, 10) ^ ROT(bb, 18) ^ ROT(bb, 24);
        return c;
    }
    ...
    private long sm4CalcRK(long ka) {
        long bb;
        long rk;
        byte[] a = new byte[4];
        byte[] b = new byte[4];
    ...
    private void sm4_setkey(long[] SK, byte[] key) {
        ...
        k[3] = MK[3] ^ (long) FK[3];
        for (; i < 32; i++) {
            k[(i + 4)] = (k[i] ^ sm4CalcRK(k[(i + 1)] ^ k[(i + 2)] ^ k[(i + 3)] ^ (long) CK[i]));
            SK[i] = k[(i + 4)];
        }
    }
    ...
    private void sm4_one_round(long[] sk, byte[] input, byte[] output) {
        ...
        ulbuf[0] = GET_LONG_BE(input, 0);
        ulbuf[1] = GET_LONG_BE(input, 4);
        ulbuf[2] = GET_LONG_BE(input, 8);
        ulbuf[3] = GET_LONG_BE(input, 12);
        while (i < 32) {
            ulbuf[(i + 4)] = sm4F(ulbuf[i], ulbuf[(i + 1)], ulbuf[(i + 2)], ulbuf[(i + 3)], sk[i]);
            i++;
        }
        PUT_LONG_BE(ulbuf[35], output, 0);
        PUT_LONG_BE(ulbuf[34], output, 4);
        PUT_LONG_BE(ulbuf[33], output, 8);
        PUT_LONG_BE(ulbuf[32], output, 12);
    }
    ...
    public void sm4_stake_dec(SM4_Context ctx, byte[] key) throws Exception {
        if (ctx == null) {
            throw new Exception("ctx is null!");
        }
